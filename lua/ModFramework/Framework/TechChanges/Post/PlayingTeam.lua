...
function PlayingTeam:InitTechTree()
	...
	for _, value in ipairs(activationToAdd) do
		Mod.Logger:PrintDebug("Adding activation node: " .. (EnumToString(kTechId, value[1]) or value[1]) .. ", Team: " .. self:GetTeamNumber(), "all")
		self.techTree:AddActivation(value[1], value[2], value[3])
	end

	for _, value in ipairs(buyToAdd) do
		Mod.Logger:PrintDebug("Adding buy node: " .. (EnumToString(kTechId, value[1]) or value[1]) .. ", Team: " .. self:GetTeamNumber(), "all")
		self.techTree:AddBuyNode(value[1], value[2], value[3], value[4])
	end

	for _, value in ipairs(buildToAdd) do
		Mod.Logger:PrintDebug("Adding build node: " .. (EnumToString(kTechId, value[1]) or value[1]) .. ", Team: " .. self:GetTeamNumber(), "all")
	...
	for _, value in ipairs(upgradesToAdd) do
		if self:GetTeamNumber() == value[4] then
			Mod.Logger:PrintDebug("Adding upgrade node: " .. (EnumToString(kTechId, value[1]) or value[1]) .. ", Team: " .. self:GetTeamNumber(), "all")
